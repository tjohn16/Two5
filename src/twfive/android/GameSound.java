...
public class GameSound 
{
	...
	// Sound doesn't work currently. Going to work on this still.
	...
	private final TwentyFiveActivity activity;
	
	private boolean musicPlayable;
	
	
	public GameSound(TwentyFiveActivity twentyfive, SharedPreferences saved) 
	{
		this.activity = twentyfive;
		this.savedVars = saved;
		...
		// NEEDS UPDATE. New sounds implemented?
		...
		musicPlayer = MediaPlayer.create(activity, R.raw.match);
	...
	{
		if (!savedVars.getBoolean("enableMusic", true) || !musicPlayable) 
		{
			return;
		}
	...
	{
		if (isMusicEnabled() && musicPlayable && !musicPlayer.isPlaying())
		{
			float actualVolume = (float) audioManager.getStreamVolume(AudioManager.STREAM_MUSIC);
			float maxVolume = (float) audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
			float volume = actualVolume / maxVolume;
